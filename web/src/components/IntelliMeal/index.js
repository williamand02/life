...
function IntelliMeal() {
    ...
    function gerarMarmita(e) {
        e.preventDefault();
        const carb = sortiar(dayMenu.filter(el => el.classificacao === "CARBO"), carbo);
        const prot = sortiar(dayMenu.filter(el => el.classificacao === "PROTEIN"), protein);
        const vegi = sortiar(dayMenu.filter(el => el.classificacao === "VEGIE"), vegie);
    ...
    const Marmita = () => {
        if (showMarmita === true) {
            const C = marmita.carb.map((el, index) => (<p key={index}>{el.descricao}</p>))
            const P = marmita.prot.map((el, index) => (<p key={index}>{el.descricao}</p>))
            const V = marmita.vegi.map((el, index) => (<p key={index}>{el.descricao}</p>))
            return (
                <div className="container-marmita lista">
                    <div>
                        <img src={imgPrato} alt="prato"></img>
                    </div>
                    <div>{C} {P} {V}</div>
                </div>)
        }
        return (
            <div className="container-marmita"></div>
        );
    }
    return (
